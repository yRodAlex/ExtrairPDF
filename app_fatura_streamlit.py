...
st.title("Leitor de Fatura - Itau e Sicoob")
...
if uploaded_file and mes and ano:
    ...
    if datas:
        st.success(f"Total de lançamentos encontrados: {len(datas)}")
        df = pd.DataFrame({
            "Data": datas,
        ...
        st.download_button(label="📥 Baixar Excel Gerado",
                           data=output,
                           file_name=f'Fatura_Completa_{banco}_{mes}_{ano}.xlsx',
                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    else:
        st.warning("Nenhum lançamento encontrado. Confira o PDF enviado.")
